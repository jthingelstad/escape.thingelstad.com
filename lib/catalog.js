...
function resolveRoomId(fields, fallbackId) {
  const explicitId = toNumber(fields.get('Room ID'));
...
function buildCatalog(rawData, options = {}) {
  ...
  const players = rawPlayers.map((record) => ({
    airtableId: record.id,
    slug: makeUniqueSlug('player', record.fields?.Name, record.id, slugRegistry),
  ...
  const trips = rawTrips.map((record) => ({
    airtableId: record.id,
    slug: makeUniqueSlug('trip', record.fields?.Name, record.id, slugRegistry),
  ...
  visibleRoomRecords.forEach((record, index) => {
    roomIdByAirtableId[record.airtableId] = resolveRoomId(record.fields, index + 1);
  });
  ...
  const rooms = visibleRoomRecords.map((record, index) => {
    ...
    const roomId = resolveRoomId(fields, index + 1);
    const game = fields.get('Game') || 'Unknown Room';
    ...
    const room = {
      ...
      slug: roomSlug(roomId, game),
      game,
      date: fields.get('When') || null,
      status: PUBLIC_STATUSES.has(fields.get('Status')) ? fields.get('Status') : 'Completed',
  ...
  const finalizedPlayers = finalizeCounts(players)
    .map((player) => ({
      ...player,
  ...
  const featured = {
    ...
    awards: finalizedAwards.filter((award) => award.featured),
