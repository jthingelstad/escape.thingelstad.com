...
function validateVisibleRoomRecords(records, lookups) {
  ...
  records.forEach((record) => {
    ...
    if (!Number.isSafeInteger(roomId) || roomId <= 0) {
      throw new Error(`Room ${record.airtableId} (${label}) must have a positive integer Room ID.`);
    }
    if (roomIds.has(roomId)) {
      throw new Error(
        `Room ID ${roomId} is duplicated by ${record.airtableId} (${label}) and ${roomIds.get(roomId)}.`
      );
    }
...
function buildCatalog(rawData, options = {}) {
  ...
  const rooms = visibleRoomRecords.map((record) => {
    const fields = record.fields;
    const roomId = resolveRoomId(fields);
